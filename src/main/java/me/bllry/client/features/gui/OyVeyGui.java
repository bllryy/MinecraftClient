...
public class OyVeyGui extends Screen {
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : bllry.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    bllry.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
